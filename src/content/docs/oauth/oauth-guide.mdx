...
:::caution[Important]
Do not expose your Client Secret in frontend code. It must only be used on trusted backend servers.
:::

:::note
* Download your Client ID and Secret immediately. They will never be shown to you again.
* Use a desktop browser for the best developer experience.
:::
...
<PackageManagers 
  pkg={['@heavstal/api', 'next-auth']} 
  githubRepo="HeavstalTech/heavstal-auth#main"
/>
...
<Tabs syncKey="code-language">
  <TabItem label="OIDC" icon="seti:javascript>
  ```typescript
  // Default
  ...
  <TabItem label="OAuth 2.0" icon="seti:javascript>
  ```typescript
  import NextAuth from "next-auth";
  import HeavstalProvider from "@heavstal/auth";
...
<Tabs syncKey="code-language">
  <TabItem label="app.js" icon="seti:javascript>
```javascript
...
<Tabs syncKey="code-language">
  <TabItem label="app.py" icon="seti:python>
```python
...
<Tabs syncKey="code-language">
  <TabItem label="auth.js" icon="seti:javascript>
```javascript
import NextAuth from "next-auth";
import HeavstalProvider from "@heavstal/auth";
...
:::note
* Ensure your application uses the new client secret before you disable the old one.
* Do not expose your Client Secret in frontend code. Use it only on trusted backend servers.
* Use a desktop browser for the best developer experience.
:::
</details>
...
:::tip
Use this method for applications with URIs spanning different domains.
:::

:::note
* If your application has multiple redirect URIs, add the HTML file to the root of all associated domains.
* Ensure your application does not block the custom `HeavstalBot/1.0` User-Agent.
:::
...
:::note
Due to DNS caching, it may take up to 20 minutes for Heavstal Auth to discover the TXT record.
:::
