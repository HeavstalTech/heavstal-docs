...
> **Note 1:** It is recommended to download these details as they will *NEVER* be shown to you again.  
> **Note 2:** Never expose your Client Secret in frontend code. It must only be used on trusted backend servers.  
> **Note 3:** It is recommended to use Desktop Site for the best developer experience and to ensure a more accurate positioning of the instructions above.
...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  callbacks: { 
    ...
    // Store the user's ID in JWT so it can be accessible throughout your application 
...
<details> 
 <summary><b>How can I rotate my OAuth credentials?</b> </summary>
  ...
  <Steps>
  1. Visit [Heavstal Auth Manager](https://heavstal.com.ng/oauth/apps).
  2. From the application list, find the app whose credentials you want to rotate and at the top right of that app select the **more options menu** and from the list select **Configure**.
  ...
  4. At the top right of the OAuth Credentials section select **+ Add Secret** to create a new client secret.
  5. A message modal will appear with your new secret, you're to copy it and replace with the old version in your app.
  6. Importantly after successfully migrating to the new client secret, you must return to the application configuration in Heavstal Authentication Manger. From the two available client secrets, find the old one (you can easily identify it via the date provided) that's usually at the bottom, and select the blue **Enabled** tag at the end of the secret to disable the Old Client secret and optionally delete it.
  </Steps>
...
You can verify your OAuth Domain by following the steps below:
  ...
  <Steps>
  1. Visit [Heavstal Auth Manager](https://heavstal.com.ng/oauth/apps).
  2. From the application list, find the application you want to verify which is usually tagged as **Test** then select the **more options menu** at the top right of the application and select **Configure**.
  3. At the top of the application you'll see a hard to miss message **"This OAuth Property is not yet verified"** with a button **Start Verification**. Select it.
  4. A pop-up window will appear asking you select a verification method which comprises of only `Verify via HTML` or `Verify via TXT Record`. Select the method best suitable to you and your application.
  </Steps>
  
  **METHOD 1: Verification via HTML**
  When verifying via HTML, the pop up window usually changes to new window asking to add a very specific HTML file to the root of your website. You are also provided with an option to download the HTML file with its contents.
  
  > **Note 1:** if your application has more than one redirect URIs added during registration, you will have to add the html file to all the redirect URIs root.  
  ...
  **METHOD 2: Verification via TXT Records**
  When verifying via TXT Records, the pop up window usually changes to a new window asking you to add a very specific TXT record to your registrar.
  
  > **Note 1:** Due to DNS caching, it may take up to 15-20 minutes for Heavstal Auth to discover the TXT record.
  
  After adding the records with either method, return to Heavstal Auth Manager and select **Verify**. Heavstal Auth will verify if the records are available and correct and update your OAuth Application to **Production.**
  
  Typical error messages include:
  - **blocked:** you get this error message when your website is blocking our Bot from successfully fetching the records, you can usually fix this by dropping the block or bot-protection (Cloudflare) temporarily and trying again.
  - **not found:** you oftentimes get this error when the provided HTML or TXT Record has not yet been added or deployed to the root of your website.
  - **mismatch:** you only get this if our Bot successfully fetches the required HTML or TXT Records but the contents gotten does not match what was given to you. 
  - **server error:** this only happens when Heavstal Bot hits an unknown type of error, typically an error above the code of "500".
  - **unreachable / lookup_error:** this if oftentimes an Internal Server Error and takes the power away from your hands, you can only fix this by contacting [Heavstal Support](https://support.heavstal.com.ng).
</details>
...
<details> 
 ...
 <br/><br/>
 Another case in which an OAuth Application gets paused is when the OAuth application's **Redirect URI or Authorized JavaScript Origins** was changed to a new **Domain** and was not re-verified within 80 days from the day of change.
 </details>

<details> 
 <summary><b>How do I resume/unpause a paused OAuth Application?</b></summary>
In order to resume or unpause an OAuth application that has been paused due to long-standing unverification, you have to go through the verification process and get the OAuth Domain verified.
</details>
